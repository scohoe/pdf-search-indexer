...
function pdf_search_indexer_settings_html() {
    ...
    <div class="wrap">
        ...
        // In the pdf_search_indexer_settings_html function, replace the display grid section with this improved version
        ...
        <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 20px; margin-bottom: 20px;">
            <div>
                ...
                <div style="margin-top: 10px; padding: 8px; background: #e7f7ff; border: 1px solid #b0e0ff; border-radius: 3px;">
                    ...
                    <p style="margin: 5px 0 0 0;">Next batch scheduled for: <?php echo date('H:i:s', $next_scheduled); ?></p>
                    ...
                    ?>
                    <p style="margin: 5px 0 0 0;">Current batch: <?php echo $progress['batch_number']; ?></p>
                    <p style="margin: 5px 0 0 0;">Processed: <?php echo $progress['processed_count']; ?> of <?php echo $progress['total_count']; ?> files</p>
                    <p style="margin: 5px 0 0 0;">Started at: <?php echo $progress['started_at']; ?></p>
                    <p style="margin: 5px 0 0 0;">Last activity: <?php echo $progress['last_update']; ?></p>
                    <?php endif; ?>
                </div>
                <?php endif; ?>
            </div>
            ...
            <div>
                ...
                <div style="max-height: 200px; overflow-y: auto; border: 1px solid #ddd; padding: 10px; background: #f9f9f9;">
                    <h4 style="margin-top: 0;">Currently Processing Files:</h4>
                ...
                <?php else: ?>
                    ...
                    <div style="border: 1px solid #ddd; padding: 10px; background: #f9f9f9;">
                        <p>The background process is active but no files are currently being processed.</p>
                        <p>This usually means one of the following:</p>
                        <ul style="margin: 5px 0; padding-left: 20px;">
                            <li>The process is preparing to start the next batch</li>
                            <li>Files are being queued for processing</li>
                            <li>The previous batch just completed</li>
                        </ul>
                        <p>The next batch will start at: <strong><?php echo date('H:i:s', $next_scheduled); ?></strong></p>
                        
                        <?php if (!empty($progress['current_file'])): ?>
                        <p>Last file processed: <strong><?php echo esc_html($progress['current_file']); ?></strong></p>
                        <?php endif; ?>
                    </div>
                    <?php else: ?>
                    <div style="border: 1px solid #ddd; padding: 10px; background: #f9f9f9;">
                        <p>No files are currently being processed.</p>
                        <p>Use the "Start Indexing" button below to begin processing PDF files.</p>
                    </div>
                    <?php endif; ?>
                <?php endif; ?>
                ...
                <div style="margin-top: 15px; border: 1px solid #ddd; padding: 10px; background: #f9f9f9;">
                    <h4 style="margin-top: 0;">Recently Completed:</h4>
                    <ul style="margin: 0; padding-left: 20px;">
                    <?php foreach ($recent_pdfs as $pdf): 
                        $file_path = get_attached_file($pdf->ID);
                        $filename = basename($file_path);
                        $status = get_post_meta($pdf->ID, '_pdf_indexing_status', true);
                        $completed_date = get_post_meta($pdf->ID, '_pdf_indexed_date', true);
                        $status_label = ($status == 'secured') ? 'Secured (cannot be indexed)' : 'Indexed successfully';
                    ?>
                        <li>
                            <strong><?php echo esc_html($filename); ?></strong><br>
                            <span style="font-size: 12px; color: #666;">
                                <?php echo esc_html($status_label); ?> at <?php echo esc_html($completed_date); ?>
                            </span>
                        </li>
                    <?php endforeach; ?>
                    </ul>
...
// Settings page HTML function is already defined in your code
// Add a function to detect and fix stalled processes
function pdf_search_indexer_check_stalled_processes() {
    // Get PDFs marked as processing
    $args = array(
        'post_type' => 'attachment',
        'post_mime_type' => 'application/pdf',
        'posts_per_page' => -1,
    ...
    if (!empty($processing_pdfs)) {
        ...
        foreach ($processing_pdfs as $pdf) {
            ...
            // If no update time or updated more than 10 minutes ago, consider it stalled
            if (!$last_updated || (strtotime($last_updated) < (current_time('timestamp') - 600))) {
                $stalled_pdfs[] = $pdf->ID;
            }
        }
        
        // If we found stalled PDFs, reset them and reschedule processing
        if (!empty($stalled_pdfs)) {
            foreach ($stalled_pdfs as $pdf_id) {
                update_post_meta($pdf_id, '_pdf_indexing_status', 'pending');
            }
            
            // Check if there's already a scheduled batch process
            $next_scheduled = wp_next_scheduled('pdf_search_indexer_batch_process');
            if (!$next_scheduled) {
                // Schedule a new batch process
                wp_schedule_single_event(time() + 30, 'pdf_search_indexer_batch_process');
            }
            
            error_log("PDF Search Indexer: Reset " . count($stalled_pdfs) . " stalled PDFs to pending status");
        }
    }
}

// Run the stalled process check hourly
add_action('pdf_search_indexer_check_stalled', 'pdf_search_indexer_check_stalled_processes');

// Schedule the stalled process check if not already scheduled
function pdf_search_indexer_schedule_checks() {
    if (!wp_next_scheduled('pdf_search_indexer_check_stalled')) {
        wp_schedule_event(time(), 'hourly', 'pdf_search_indexer_check_stalled');
    }
}
add_action('admin_init', 'pdf_search_indexer_schedule_checks');

// Clear scheduled events on plugin deactivation
function pdf_search_indexer_deactivate() {
    wp_clear_scheduled_hook('pdf_search_indexer_check_stalled');
    wp_clear_scheduled_hook('pdf_search_indexer_batch_process');
}
